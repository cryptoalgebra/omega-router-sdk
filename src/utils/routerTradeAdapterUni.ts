...
// Integral SDK вместо uniswap v4
import { Pool as IntegralPool, Route as IntegralRoute, Trade } from '@cryptoalgebra/integral-sdk'
...
export enum PoolType {
  V2Pool = 'v2-pool',
  V3Pool = 'v3-pool',
  IntegralPool = 'integral-pool', // вместо v4-pool
}
...
export type PartialClassicQuote = {
  ...
  route: Array<(IntegralPoolInRoute | V3PoolInRoute | V2PoolInRoute)[]>
}

interface RouteResult {
  // помним: в результате мы всё ещё подаём эти routes в RouterTrade.v4Routes,
  // но сами объекты — IntegralRoute/IntegralPool
...
export class RouterTradeAdapter {
  ...
  static fromClassicQuote(quote: PartialClassicQuote) {
    ...
    const typedRoutes: RouteResult[] = route.map((subRoute) => {
      ...
      const isOnlyIntegral = RouterTradeAdapter.isVersionedRoute<IntegralPoolInRoute>(PoolType.IntegralPool, subRoute)
      return {
        routeIntegral: isOnlyIntegral
          ? new IntegralRoute(
              (subRoute as IntegralPoolInRoute[]).map(RouterTradeAdapter.toIntegralPool),
              parsedCurrencyIn,
              parsedCurrencyOut
            )
          : null,
        ...
        routev2: isOnlyV2
          ...
          : null,
        mixedRoute:
          !isOnlyIntegral && !isOnlyV3 && !isOnlyV2
    ...
    const integralRoute = typedRoutes.filter((route) => route.routeIntegral)[0]

    return Trade.createUncheckedTrade({
      route: integralRoute.routeIntegral as IntegralRoute<Currency, Currency>,
      inputAmount: integralRoute.inputAmount,
      outputAmount: integralRoute.outputAmount,
      tradeType: quote.tradeType,
    })

    // NOTE: RouterTrade expects v4Routes key — оставляем имя поля v4Routes, но в нём лежат IntegralRoute объекты.
    return new RouterTrade({
      ...
      // сюда подсовываем IntegralRoute, чтобы внешняя логика, ожидающая v4Routes, продолжила работать
      v4Routes: typedRoutes
        .filter((route) => route.routeIntegral)
        .map((route) => ({
          routev4: route.routeIntegral as IntegralRoute<Currency, Currency>,
          inputAmount: route.inputAmount,
          outputAmount: route.outputAmount,
        })),
  ...
  private static toPoolOrPair = (
    pool: IntegralPoolInRoute | V3PoolInRoute | V2PoolInRoute
  ): IntegralPool | V3Pool | Pair => {
    switch (pool.type) {
      case PoolType.IntegralPool:
        return RouterTradeAdapter.toIntegralPool(pool as IntegralPoolInRoute)
      case PoolType.V3Pool:
        return RouterTradeAdapter.toV3Pool(pool as V3PoolInRoute)
      case PoolType.V2Pool:
        return RouterTradeAdapter.toPair(pool as V2PoolInRoute)
      default:
  ...
  // новая функция: собираем IntegralPool из данных маршрута
  private static toIntegralPool(pool: IntegralPoolInRoute): IntegralPool {
    const parsedCurrencyIn = RouterTradeAdapter.toCurrency(isNativeCurrency(pool.tokenIn.address), pool.tokenIn).wrapped
    const parsedCurrencyOut = RouterTradeAdapter.toCurrency(
      isNativeCurrency(pool.tokenOut.address),
      pool.tokenOut
    ).wrapped
    // Конструктор IntegralPool подобен v4: (currency0, currency1, fee, tickSpacing, hooks, sqrtRatioX96, liquidity, tickCurrent)
    // Если в твоём SDK сигнатура отличается — подправь соответствующие поля.
  ...
  private static isVersionedRoute<T extends V2PoolInRoute | V3PoolInRoute | IntegralPoolInRoute>(
    type: PoolType,
    route: (V3PoolInRoute | V2PoolInRoute | IntegralPoolInRoute)[]
  ): route is T[] {
